...
if __name__ == '__main__':
    ...
    data_root = '/home/qtx/Datasets/TRAJECTORIES'
    data_reg = 'egfr'
    all_type = 'all'
    traj_nums = 24
    batch_size = 16
    ...
    egfr_dataset = EGFR(data_root, traj_nums=traj_nums, groups=groups, folds=folds, val_fold_idx=0, dataset_arg=data_reg, atom_type=all_type)
    print(egfr_dataset.raw_file_names)

    idx_train, idx_val, idx_test = egfr_dataset.get_idx_split()
    pdb.set_trace()
    train_loader = DataLoader(Subset(egfr_dataset, idx_train), batch_size=groups * (folds - 1), num_workers=num_workers, shuffle=False, drop_last=False)
    val_loader = DataLoader(Subset(egfr_dataset, idx_val), batch_size=groups, num_workers=num_workers, shuffle=False, drop_last=False)
    test_loader = DataLoader(Subset(egfr_dataset, idx_test), batch_size=groups * folds, num_workers=num_workers, shuffle=False, drop_last=False)

    for epoch in range(1):
        for idx, data in enumerate(train_loader):
            print('Epoch:{} || Iteration:{}, data:{}'.format(epoch, idx, data))
            # print('Epoch:{} || Iteration:{}, data shape:{}, label shape:{}'.format(epoch, idx, cur_x.shape, cur_y.shape))
            print(torch.min(data.pos), torch.max(data.pos))
            # print(data.pos)
            print(data.y)

    for idx, data in enumerate(val_loader):
        print('Val || Iteration:{}, data shape:{}, label shape:{}'.format(idx, data.pos.shape, data.y.shape))
        print(torch.min(data.pos), torch.max(data.pos))
        # print(data.pos)
        print(data.y)

    for idx, data in enumerate(test_loader):
        print('Test || Iteration:{}, data shape:{}, label shape:{}'.format(idx, data.pos.shape, data.y.shape))
        print(torch.min(data.pos), torch.max(data.pos))
        # print(data.pos)
        print(data.y)
